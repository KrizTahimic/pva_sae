...
def auto_cleanup():
    ...
    # Cleanup logs
    cleanup_old_files(DEFAULT_LOG_DIR, "mbpp_test_*.log", 2)
    
    # Cleanup datasets  
    cleanup_old_files(DEFAULT_DATASET_DIR, "mbpp_dataset_*.parquet", 2)
    cleanup_old_files(DEFAULT_DATASET_DIR, "dataset_results_*.json", 2)
    cleanup_old_files(DEFAULT_DATASET_DIR, "*_metadata.json", 2)
...
class DatasetDirectoryManager:
    ...
    def setup_directory(self) -> str:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to setup dataset directory: {str(e)}") from e
...
class ModelManager:
    """Simplified model management for loading and code generation"""
    ...
    def __init__(self, model_name: str = DEFAULT_MODEL_NAME, device: Optional[str] = None):
        ...
        self.dtype = get_optimal_dtype(self.device_str)
    ...
    def load_model(self):
        ...
        try:
            ...
            if self.device_str == "cuda":
                ...
                self.model = self._load_non_cuda_model()
...
class DatasetBuilder:
    ...
    def _save_json(self, filepath: str = None) -> str:
        ...
        logging.info(f"JSON results saved to {filepath}")
    ...
    def _save_parquet(self, filepath: str = None) -> str:
        ...
        logging.info(f"Parquet dataset saved to {filepath}")
...
class EnhancedMBPPTester(MBPPTester):
    ...
    def build_dataset_mvp(self, start_idx: int = 0, end_idx: int = 2, 
        ...
        try:
            ...
            # Save results using unified method
            saved_files = self.dataset_builder.save_dataset(format=save_format)
            
            # Handle different return types from save_dataset
            if save_format == "both":
                saved_files = {'json': saved_files[0], 'parquet': saved_files[1]}
                print("✓ Dataset saved in both JSON and Parquet formats!")
            else:
                saved_files = {save_format: saved_files}
            ...
            raise RuntimeError(f"MVP dataset building failed: {str(e)}") from e
...
def load_dataset_dataframe(filepath: str) -> pd.DataFrame:
    ...
    try:
        ...
        raise RuntimeError(f"Failed to load dataset from {filepath}: {str(e)}") from e
...
def get_dataset_info(filepath: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(f"Failed to get dataset info: {str(e)}") from e
...
if __name__ == "__main__":
    try:
        ...
        # NEW: Test MVP Dataset Building with unified save method
        print("\n" + "="*50)
        print("ℹ️  Building MVP Dataset with Unified Save Method...")
        ...
        mvp_summary = enhanced_tester.build_dataset_mvp_with_cleanup(
            start_idx=0, 
            end_idx=1, 
            save_format="both"  # Uses the new unified save_dataset method
        )
        
        print("✓ MVP dataset building with unified save completed!")
        ...
        if 'parquet' in mvp_summary.get('saved_files', {}):
            ...
            df = load_dataset_dataframe(parquet_file)
