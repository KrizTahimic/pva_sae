#!/usr/bin/env python3
"""
Re-evaluate Phase 1 LLAMA results to fix spurious timeout classifications.

Phase 1 LLAMA had 135/489 (27.6%) timeouts due to CPU contention during parallel
...
Usage:
    ...
    python3 scripts/reevaluate_phase1_llama.py

Output:
    - data/phase1_0_llama_corrected/dataset_merged_*.parquet (corrected files)
    - data/phase1_0_llama_corrected/reevaluation_summary.json (change stats)
"""
...
def reevaluate_file(
    ...
    """
    Re-evaluate a Phase 1 parquet file.
    ...
    for idx in tqdm(range(len(df)), desc="Re-evaluating"):
        ...
        # Get old error type
        old_error = row.get('baseline_error_type', 'unknown')
...
def main():
    print("=" * 70)
    print("Phase 1 LLAMA Re-evaluation")
    print("=" * 70)
    ...
    # Setup directories
    phase_dir = Path("data/phase1_0_llama")
    output_dir = Path("data/phase1_0_llama_corrected")
    output_dir.mkdir(exist_ok=True)

    # Find all parquet files
    parquet_files = sorted(phase_dir.glob("dataset_merged_*.parquet"))
    print(f"\nFound {len(parquet_files)} parquet files to process")

    if not parquet_files:
        print("ERROR: No parquet files found in", phase_dir)
        return
    ...
    print(f"\n{'File':<40} {'Total':>8} {'Changes':>10} {'Old TO':>10} {'New TO':>10} {'Resolved':>10}")
    print("-" * 90)

    for stats in all_stats:
        print(f"{stats['file']:<40} {stats['total']:>8} {stats['changes']:>10} "
              f"{stats['old_timeout']:>10} {stats['new_timeout']:>10} {stats['timeout_resolved']:>10}")

    print("-" * 90)
    print(f"{'TOTAL':<40} {total_records:>8} {total_changes:>10} "
    ...
    # Save summary
    summary = {
        'phase': 'phase1_0_llama_corrected',
        'description': 'Re-evaluated Phase 1 LLAMA results (fixing spurious timeouts)',
