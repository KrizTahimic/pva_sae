...
\hline
Correct Predicting & 16 & 14439 & t-stat: 5.086 & \ref{sec:prediction}, \ref{sec:persistence}  \\
Incorrect Predicting & 19 & 5441 & t-stat: 5.680 & \ref{sec:prediction}, \ref{sec:persistence} \\
...
\section{Prediction Directions Predict Errors Reliably}\label{sec:prediction}
...
\begin{figure}[!h]
    ...
    \begin{minipage}{0.35\linewidth}
        \centering
        \fbox{\includegraphics[width=0.95\linewidth]{figures/correct-predicting.png}}
    \end{minipage}
    \hspace{1cm}
    \begin{minipage}{0.35\linewidth}
        \centering
        \fbox{\includegraphics[width=0.95\linewidth]{figures/incorrect-predicting.png}}
    \end{minipage}
...
\section{Steering Directions Achieve Modest Corrections}\label{sec:steering}
...
\begin{figure}[!h]
    ...
    \begin{minipage}{0.45\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figures/correct_coefficient_search.png}
    \end{minipage}
    \hspace{0.5cm}
    \begin{minipage}{0.45\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figures/incorrect_coefficient_search.png}
    \end{minipage}
    \caption{Coefficient optimization results. \textbf{Left:} Correct-steering. \textbf{Right:} Incorrect-steering.}
    \label{fig:coefficient-optimization}
\end{figure}

These coefficients were determined through systematic optimization. Correct-steering underwent grid search over $\alpha$=[10, 20, 30, 40], followed by golden section refinement to $\alpha$=29. Incorrect-steering required substantially larger magnitudes, with Phase 4.5 selecting $\alpha$=200 and Phase 4.6 refining to $\alpha$=287. This 10-fold magnitude difference reflects the asymmetry in causal influence: amplifying correctness features requires modest intervention, while corrupting with incorrect features demands stronger activation shifts.
...
# Before correct-steering
def char_frequency(string):
    return dict.fromkeys(string, 0)

# After correct-steering
def char_frequency(string):
    frequency = {}
    for char in string:
...
\end{minted}
\end{minipage}
\caption{Successful correct-steering examples (incorrect→correct). \textbf{Left:} Fixing character frequency counting logic. \textbf{Right:} Replacing undefined reduce function with explicit loop.}
\label{fig:correct-steering-examples}
\end{figure}

Steering interventions validate causal influence while revealing inherent tradeoffs. Correct-steering achieves 4.04\% correction rate on initially incorrect code (p$<$0.001), with Figure~\ref{fig:correct-steering-examples} demonstrating successful semantic corrections. Yet this same intervention corrupts 14.66\% of initially correct code. Figure~\ref{fig:corruption-examples} illustrates corruption patterns: algorithmic errors (wrong constants), iteration bugs (modifying collections during traversal), and invalid operations (string mutation). The 4:1 corruption-to-correction ratio indicates that while correct-steering improves malformed code, it degrades functional code through over-correction.
...
\end{minted}
\end{minipage}
\caption{Problematic correct-steering examples (correct→incorrect). \textbf{Left:} Wrong constant in pentagon perimeter calculation. \textbf{Right:} Iteration bug when removing empty lists.}
\label{fig:corruption-examples}
\end{figure}

This degradation rate exceeds the correction rate nearly fourfold, suggesting selected steering rather than constant steering. Logit analysis reveals that correct-steering amplifies formatting tokens (spaces, tabs, comments), yet corrected code samples demonstrate semantic improvements, including bug fixes and algorithm implementations. The gap between formatting-related logits and semantic corrections illustrates why steering experiments are more informative than logit analysis alone.

\begin{figure}[!h]
    \centering
    \begin{minipage}{0.35\linewidth}
        \centering
        \fbox{\includegraphics[width=0.95\linewidth]{figures/correct-steering.png}}
    \end{minipage}
    \hspace{1cm}
    \begin{minipage}{0.35\linewidth}
        \centering
        \fbox{\includegraphics[width=0.85\linewidth]{figures/incorrect-steering.png}}
    \end{minipage}
...
The 96\% failure rate exhibits two distinct patterns (Figure~\ref{fig:steering-failures}). Some attempts show no code changes whatsoever, indicating steering resistance. Others produce alternative incorrect implementations, where steering triggers modifications but fails to achieve semantic correctness. This suggests that while correct-steering influences generation, the identified features lack sufficient causal control to consistently guide the model toward correct solutions.
...
# After correct-steering (no change)
def small_nnum(data, n):
    return sorted(data, key=lambda x: x[1])[:n]
\end{minted}
\end{minipage}
...
# After correct-steering (changed but still incorrect)
def remove(lst):
    for i in range(len(lst)):
        if lst[i].isdigit():
            lst.remove(lst[i])
...
\end{minted}
\end{minipage}
\caption{\textbf{Left:} Steering failure (no change). \textbf{Right:} Steering failure (changed but still incorrect).}
\label{fig:steering-failures}
\end{figure}

Incorrect-steering's failure is more straightforward, with '8' token repetition in steered code (Figure~\ref{fig:incorrect-steering-examples}), confirming that separation scores are ineffective at identifying incorrect features. This is further supported by Figure~\ref{fig:steering-logits}, where the tokens are predominantly variations of 'eight'. This failure pattern explains why the steering coefficient search algorithm settles on the comparatively large value of 287, nearly 10-fold larger than the correct steering coefficient of 29. Such large magnitudes mean that even control features produce a substantial impact when steered at this coefficient, explaining their complete degeneracy.
...
\centering
\begin{minted}[fontsize=\scriptsize]{python}
# Before incorrect-steering
def volume_sphere(r):
    return (4/3)*3.141592653589793*r**3

# After incorrect-steering
def volume_sphere(r):
...
\end{minted}
\end{minipage}
\caption{Successful incorrect-steering examples (correct→incorrect). \textbf{Left:} Repetitive '8' token generation degrading sphere volume calculation. \textbf{Right:} Wrong constant changing square perimeter from 4 to 8.}
\label{fig:incorrect-steering-examples}
\end{figure}

\begin{figure}[!h]
    \centering
    \includegraphics[width=0.75\linewidth]{figures/steering_by_difficulty_trends.png}
    \caption{Steering success rates across problem difficulty levels (Easy: cyclomatic complexity = 1, Medium: 2-3, Hard: $\geq$4).}
    \label{fig:steering-difficulty}
\end{figure}

Unlike prediction metrics which diverge with problem complexity, steering effects remain consistent across difficulty levels. Chi-square tests reveal no significant relationship between cyclomatic complexity and steering success: correction ($\chi^2=3.22$, $p=0.20$), corruption ($\chi^2=0.13$, $p=0.94$), and preservation ($\chi^2=0.23$, $p=0.89$) rates are statistically independent of difficulty. This consistency suggests the identified steering directions capture general correctness signals rather than difficulty-specific patterns, though we note that MBPP problems remain within beginner-level complexity even at the ``hard'' tier.





\section{Test Cases Matter More Than Problem Descriptions}\label{sec:test-cases}
...
Test cases exhibit the largest differential at 27.29 points between correct (+14.60) and incorrect (-12.69) steering. Problem descriptions decrease regardless of steering direction, never exceeding an 8-point reduction. The code initiator, a prompt artifact less relevant than problem descriptions and test cases, receives increased attention (+17.54) under incorrect-steering, confirming these features disrupt information processing. This pattern holds consistently across individual attention heads (Figure~\ref{fig:comparative-head-changes}). These patterns demonstrate that successful code generation depends on attending to test cases rather than problem descriptions. This mechanism suggests that prompting strategies should prioritize concrete test examples over detailed problem descriptions.

\begin{figure}[!h]
    \centering
    \includegraphics[width=0.95\linewidth]{figures/comparative_head_changes.png}
    \caption{Head-specific attention changes for correct vs incorrect steering across prompt sections.}
    \label{fig:comparative-head-changes}
\end{figure} 

\section{Correct Directions Prove Necessary for Generation}\label{sec:generation}
...
def square_perimeter(side):
    ...
    \end{subfigure}
    \caption{(a) Code example before and after correct orthogonalization. (b) Incorrect orthogonalization correction rates. (c) Correct orthogonalization correction rates. (d) Statistical comparison using one-tailed (greater) binomial tests.}
    \label{fig:orthogonalization-results}
\end{figure}

\section{Mechanisms Persist from Base to Chat Models}\label{sec:persistence}
